...
export async function GET(request) {
  try {
    ...
    const { data: material } = await supabase.from('mentorship_materials').select('id,aula_id,title,file_url').eq('id', id).maybeSingle()
    ...
    await supabase.from('app_content_access_events').insert({
      ...
      content_type: 'mentorship_material',
      content_id: material.id,
      content_title: material.title,
      metadata: { lesson_id: material.aula_id, mentorship_type: aula?.mentorship_type },
    })

...
function tituloPadrao(mesAno) {
  const [ano, mes] = String(mesAno).split('-')
  return `Calendário de ${MESES[Number(mes) - 1] || ''} ${ano}`.trim()
}

async function comLinks(supabase, itens) {
  return Promise.all((itens || []).map(async item => {
    if (item.storage_bucket === BUCKET && item.arquivo_nome) {
...
export async function GET(request) {
  ...
  return NextResponse.json({ calendarios: await comLinks(supabase, data) })
}
...
export async function POST(request) {
  ...
  try {
    ...
    const resultado = existente
      ? await supabase.from('calendario').update(registro).eq('id', existente.id).select().single()
      : await supabase.from('calendario').insert(registro).select().single()
    ...
    return NextResponse.json({ calendario: resultado.data, substituido: Boolean(existente) }, { status: 201 })
...
export async function DELETE(request) {
  ...
  const { data: item } = await supabase.from('calendario').select('*').eq('id', id).maybeSingle()
  if (!item) return NextResponse.json({ error: 'Calendário não encontrado.' }, { status: 404 })

...
export default function SalesCenter({ cores, ouro, ouroGrad, initialTab = 'painel', rotinaSemanal, onOpenRoutine }) {
  ...
  async function saveGoal(event) {
    ...
    setMessage(error ? error.message : '✓ Meta e distribuição salvas.'); setSaving(false); if (!error) { setTab('painel'); load() }
  }
  ...
  return <div className="team-goals" style={{ '--tg-gold': ouro, '--tg-text': cores.tx, '--tg-muted': cores.tx2, '--tg-border': cores.borda, '--tg-card': cores.card, '--tg-card2': cores.card2 }}>
    ...
    {tab === 'painel' && <>
      ...
      <section className="tg-hero" style={cardStyle}><div><small>META DE {monthLabel(selectedMonth).toUpperCase()}</small><strong>{brl(totalSold)}</strong><span>de {brl(target)}</span></div><div className={`tg-status ${pace}`}><b>{achievement.toFixed(0)}%</b><span>{pace === 'batida' ? 'Meta batida' : pace === 'no-ritmo' ? 'No ritmo' : pace === 'atrasada' ? 'Abaixo do ritmo' : 'Defina a meta'}</span></div><Progress value={achievement} gradient={ouroGrad} /><button className="tg-primary" onClick={() => setTab('meta')} style={{ position: 'static', gridColumn: '1 / -1', justifySelf: 'end', background: ouroGrad, color: '#211a0e', border: 0, borderRadius: 11, padding: '12px 18px', boxShadow: '0 8px 20px rgba(212,175,55,.18)' }}>⚙ Configurar meta do mês</button></section>
    ...
    {tab === 'meta' && <form className="tg-config" onSubmit={saveGoal} style={cardStyle}><div className="tg-config-title"><div><h3>Configurar meta de {monthLabel(selectedMonth)}</h3><p>Defina a meta do mês e o peso de venda de cada dia. Zero significa que a loja não abre.</p></div><button type="button" onClick={() => setGoal(current => ({ ...current, weekday_weights: DEFAULT_WEIGHTS }))}>Restaurar sugestão</button></div><label className="tg-target">Meta mensal<input autoFocus type="number" min="0" step="0.01" value={goal.monthly_target} onChange={event => setGoal({ ...goal, monthly_target: event.target.value })} style={inputStyle} /></label><div className="tg-weights">{WEEKDAYS.map(([day, label]) => <label key={day}>{label}<input type="number" min="0" max="100" value={weights[day] ?? 0} onChange={event => setWeight(day, event.target.value)} style={inputStyle} /><span>%</span></label>)}</div><p className="tg-weight-total">Soma dos pesos: <b>{Object.values(weights).reduce((sum, value) => sum + Number(value || 0), 0)}%</b> · {openDates.length} dias de venda no mês</p><button className="tg-primary" disabled={saving}>{saving ? 'Salvando...' : 'Salvar meta do mês'}</button></form>}

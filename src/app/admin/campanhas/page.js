...
export default function AdminCampanhas() {
  ...
  function limpar() { setFormAberto(false); setMesAno(mesAtual()); setTitulo(''); setDescricao(''); setArquivo(null) }
  function abrirPara(item = null) {
    setMesAno(item?.mes_ano || mesAtual()); setTitulo(item?.titulo || ''); setDescricao(item?.descricao || ''); setArquivo(null); setMensagem(''); setFormAberto(true); window.scrollTo({ top: 0, behavior: 'smooth' })
  }
  async function enviar(evento) {
    evento.preventDefault()
    if (!arquivo) { setMensagem('Escolha o arquivo PDF.'); return }
    if (arquivo.type !== 'application/pdf') { setMensagem('Envie somente um arquivo PDF.'); return }
  ...
  return <div style={{ minHeight: '100vh', background: '#0A0A0A', color: '#FFF', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif' }}>
    ...
    <main style={{ maxWidth: '920px', margin: '0 auto', padding: '26px 18px 60px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: '16px', alignItems: 'flex-end', flexWrap: 'wrap', marginBottom: '22px' }}>
        <div><p style={{ color: ouro, fontSize: '11px', fontWeight: 800, letterSpacing: '.12em', margin: 0 }}>ADMINISTRAÇÃO</p><h1 style={{ fontSize: '24px', margin: '5px 0' }}>Campanha mensal</h1><p style={{ color: '#888', margin: 0 }}>Publique campanhas prontas em PDF para as alunas aplicarem nas vendas.</p></div>
        {!formAberto && <button onClick={() => abrirPara()} style={{ background: ouroGrad, color: '#090909', border: 0, borderRadius: '9px', padding: '11px 17px', fontWeight: 900, cursor: 'pointer' }}>+ Publicar campanha</button>}
      </div>
      ...
      {formAberto && <form onSubmit={enviar} style={{ background: '#111', border: '1px solid #34302A', borderRadius: '16px', padding: '20px', marginBottom: '22px' }}>
        <h2 style={{ fontSize: '17px', margin: '0 0 5px' }}>Publicar campanha</h2><p style={{ color: '#777', fontSize: '12px', margin: '0 0 17px' }}>Se já existir um PDF no mês escolhido, ele será substituído.</p>
        <label style={{ display: 'block', border: '1px dashed #66561e', background: '#0A0A0A', borderRadius: '12px', padding: '26px 16px', textAlign: 'center', cursor: 'pointer', marginBottom: '15px' }}><strong style={{ color: ouro, display: 'block', marginBottom: '5px' }}>{arquivo ? `📄 ${arquivo.name}` : '↑ Escolher o PDF da campanha'}</strong><small style={{ color: '#777' }}>{arquivo ? `${(arquivo.size / 1024 / 1024).toFixed(1)} MB` : 'Arquivo PDF de até 20 MB'}</small><input type="file" accept="application/pdf" onChange={e => setArquivo(e.target.files?.[0] || null)} style={{ display: 'none' }} /></label>
      ...
      <section style={{ display: 'grid', gap: '12px' }}>
        {itens.map(item => <article key={item.id} style={{ background: '#111', border: '1px solid #2A2A2A', borderLeft: `3px solid ${ouro}`, borderRadius: '14px', padding: '16px', display: 'flex', alignItems: 'center', gap: '15px', flexWrap: 'wrap' }}>
          ...
          <div style={{ flex: 1, minWidth: '190px' }}><small style={{ color: ouro, fontWeight: 800, textTransform: 'uppercase' }}>{rotuloMes(item.mes_ano)}</small><h3 style={{ fontSize: '15px', margin: '4px 0 3px' }}>{item.titulo}</h3><p style={{ color: '#777', fontSize: '12px', margin: 0 }}>{item.descricao || 'Campanha pronta para as alunas'}</p></div>
          <div style={{ display: 'flex', gap: '7px', flexWrap: 'wrap' }}>{item.arquivo_url && <a href={item.arquivo_url} target="_blank" rel="noopener noreferrer" style={{ ...botao, textDecoration: 'none' }}>Visualizar</a>}<button onClick={() => abrirPara(item)} style={botao}>Substituir</button><button onClick={() => excluir(item)} style={{ ...botao, color: '#f99' }}>Excluir</button></div>
        </article>)}

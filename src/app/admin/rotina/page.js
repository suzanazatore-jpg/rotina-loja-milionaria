...
export default function AdminRotina() {
  ...
  function fechar() { setFormAberto(false); setSemanaInicio(segundaFeiraAtual()); setTitulo(''); setDescricao(''); setArquivo(null) }
  function abrir(item = null) { setSemanaInicio(item?.semana_inicio || segundaFeiraAtual()); setTitulo(item?.titulo || ''); setDescricao(item?.descricao || ''); setArquivo(null); setMensagem(''); setFormAberto(true); window.scrollTo({ top: 0, behavior: 'smooth' }) }
  ...
  return <div style={{ minHeight: '100vh', background: '#0A0A0A', color: '#FFF', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif' }}>
    ...
    <main style={{ maxWidth: '920px', margin: '0 auto', padding: '26px 18px 60px' }}>
      ...
      {formAberto && <form onSubmit={enviar} style={{ background: '#111', border: '1px solid #34302A', borderRadius: '16px', padding: '20px', marginBottom: '22px' }}>
        <h2 style={{ fontSize: '17px', margin: '0 0 5px' }}>Publicar rotina da semana</h2><p style={{ color: '#777', fontSize: '12px', margin: '0 0 17px' }}>Se a semana já tiver um PDF, o novo arquivo substituirá o anterior.</p>

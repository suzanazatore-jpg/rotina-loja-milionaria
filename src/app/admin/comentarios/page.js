...
export default function ComentariosAdmin() {
  ...
  async function responder(item) { const body = String(respostas[item.id] || '').trim(); if (!body) return; setProcessando(item.id); try { await api('', { method: 'POST', body: JSON.stringify({ parent_id: item.id, body }) }); setRespostas(atual => ({ ...atual, [item.id]: '' })); await carregar() } catch (e) { setErro(e.message) } finally { setProcessando('') } }
  async function arquivar(item) { setProcessando(item.id); try { await api('', { method: 'PATCH', body: JSON.stringify({ id: item.id, archived: !item.archived }) }); await carregar() } catch (e) { setErro(e.message) } finally { setProcessando('') } }
  async function excluir(item) { if (!window.confirm('Excluir esta pergunta e todas as respostas? Essa ação não pode ser desfeita.')) return; setProcessando(item.id); try { await api('', { method: 'DELETE', body: JSON.stringify({ id: item.id }) }); await carregar() } catch (e) { setErro(e.message) } finally { setProcessando('') } }

...
export default function AdminCalendario() {
  ...
  async function enviar(evento) {
    ...
    if (modoForm !== 'editar' && !arquivo) { setMensagem('Escolha o arquivo PDF ou Word.'); return }
    const extensao = arquivo?.name?.split('.').pop()?.toLowerCase()
    if (arquivo && !['pdf', 'doc', 'docx'].includes(extensao)) { setMensagem('Envie um arquivo PDF ou Word (.doc ou .docx).'); return }
    if (arquivo && arquivo.size > 20 * 1024 * 1024) { setMensagem('O arquivo deve ter no máximo 20 MB.'); return }
  ...
  return <div style={{ minHeight: '100vh', background: '#0A0A0A', color: '#FFF', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif' }}>
    ...
    <main style={{ maxWidth: '920px', margin: '0 auto', padding: '26px 18px 60px' }}>
      ...
      {formAberto && <form onSubmit={enviar} style={{ background: '#111', border: '1px solid #34302A', borderRadius: '16px', padding: '20px', marginBottom: '22px' }}>
        ...
        {modoForm !== 'editar' && <label style={{ display: 'block', border: '1px dashed #66561e', background: '#0A0A0A', borderRadius: '12px', padding: '26px 16px', textAlign: 'center', cursor: 'pointer', marginBottom: '15px' }}><strong style={{ color: ouro, display: 'block', marginBottom: '5px' }}>{arquivo ? `📄 ${arquivo.name}` : '↑ Escolher PDF ou Word do calendário'}</strong><small style={{ color: '#777' }}>{arquivo ? `${(arquivo.size / 1024 / 1024).toFixed(1)} MB` : 'Arquivo PDF, DOC ou DOCX de até 20 MB'}</small><input type="file" accept=".pdf,.doc,.docx,application/pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document" onChange={e => setArquivo(e.target.files?.[0] || null)} style={{ display: 'none' }} /></label>}

...
export default function AdminAlunas() {
  ...
  // ── Estado do formulário de NOVA ALUNA ──
  ...
  // ── Abrir/cancelar formulário de nova aluna ──
  ...
  // ── Cadastrar nova aluna (chama /api/criar-aluna) ──
  async function cadastrarAluna() {
    if (!novoNome.trim() || !novoEmail.trim() || !novaSenha) {
      setMsg('⚠ Nome, e-mail e senha são obrigatórios.')
      return
    }
    ...
    try {
      // Pega o token da sessão atual do admin pra autenticar a chamada à API
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0A0A0A', color: '#FFFFFF', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif' }}>
      ...
      <header style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '14px', padding: '16px 20px', borderBottom: '1px solid #2A2A2A', background: '#111111', position: 'sticky', top: 0, zIndex: 10 }}>
        ...
        {!mostrarForm && (
          <button onClick={abrirNovaAluna} style={{ background: ouroGrad, color: '#0A0A0A', border: 'none', borderRadius: '9px', padding: '10px 16px', fontSize: '13px', fontWeight: 800, cursor: 'pointer' }}>+ Nova Aluna</button>
        )}
      </header>
      ...
      <main style={{ maxWidth: '680px', margin: '0 auto', padding: '24px 18px 60px' }}>
        ...
        {mostrarForm && (
          <div style={{ background: '#111111', border: `1px solid ${ouro}`, borderRadius: '16px', padding: '20px', marginBottom: '20px' }}>
            ...
            <div style={{ marginBottom: '14px' }}>
              <label style={{ display: 'block', fontSize: '12px', fontWeight: 700, color: '#888', marginBottom: '6px' }}>Senha * (mín. 6 caracteres)</label>
              <input type="text" value={novaSenha} onChange={e => setNovaSenha(e.target.value)} placeholder="Senha inicial"
                style={{ width: '100%', padding: '11px 13px', background: '#0A0A0A', border: '1px solid #2A2A2A', borderRadius: '9px', fontSize: '14px', color: '#FFF', outline: 'none', boxSizing: 'border-box' }} />
            </div>

            <div style={{ marginBottom: '16px' }}>
              <label style={{ display: 'block', fontSize: '12px', fontWeight: 700, color: '#888', marginBottom: '6px' }}>WhatsApp (opcional)</label>
              <input value={novoWhatsapp} onChange={e => setNovoWhatsapp(e.target.value)} placeholder="(00) 00000-0000"
                style={{ width: '100%', padding: '11px 13px', background: '#0A0A0A', border: '1px solid #2A2A2A', borderRadius: '9px', fontSize: '14px', color: '#FFF', outline: 'none', boxSizing: 'border-box' }} />
            </div>
              ...
              {editando === aluna.id ? (
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
                  ...
                  <div style={{ display: 'flex', gap: '8px', flexShrink: 0 }}>
                    ...
                    >
                      {reenviando === aluna.id ? 'Enviando...' : 'Reenviar e-mail'}
                    </button>

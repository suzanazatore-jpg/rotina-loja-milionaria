...
export default function Mentoria() {
  ...
  const cores = tema === 'escuro'
    ? { bg: '#0A0A0A', card: '#111111', card2: '#171717', borda: '#292929', borda2: '#333333', tx: '#FFFFFF', tx2: '#AAAAAA', tx3: '#777777', ativo: '#211E13', lateral: '#101010' }
    : { bg: '#F7F6F2', card: '#FFFFFF', card2: '#F3F1EB', borda: '#E2E0D8', borda2: '#D5D2C8', tx: '#1A1A18', tx2: '#6A6A62', tx3: '#8A8A82', ativo: '#FFF6D8', lateral: '#F0EFEA' }
  ...
  return (
    <div style={{ minHeight: '100vh', background: cores.bg, color: cores.tx, fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif' }}>
      <header style={{ minHeight: '64px', padding: '12px 20px', borderBottom: `1px solid ${cores.borda}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '14px', position: 'sticky', top: 0, zIndex: 20, background: cores.bg }}>
        <button onClick={() => programa ? (setPrograma(null), setAulaId(null), router.replace('/mentoria')) : router.push('/painel?secao=conteudos')} style={botaoSecundarioTema}>← {programa ? 'Mentorias' : 'Voltar aos conteúdos'}</button>
      ...
      {!programa ? <main className="mentoria-hub">
        ...
        <main style={{ padding: '26px', minWidth: 0 }}>
          {aulaAtual ? <>
            <div style={{ width: '100%', aspectRatio: '16/9', background: cores.card, border: `1px solid ${cores.borda}`, borderRadius: '16px', overflow: 'hidden', display: 'grid', placeItems: 'center' }}>
              {aulaAtual.video_url ? <iframe src={urlVideo(aulaAtual.video_url)} title={aulaAtual.titulo} allow="autoplay; fullscreen; picture-in-picture" allowFullScreen style={{ width: '100%', height: '100%', border: 0 }} /> : <span style={{ color: cores.tx3 }}>🎬 Vídeo em breve</span>}
            </div>

            <div style={{ display: 'flex', gap: 22, borderBottom: `1px solid ${cores.borda}`, marginTop: 20 }}>
              <button onClick={() => setAbaConteudo('informacoes')} style={abaStyleTema(abaConteudo === 'informacoes')}>Informações</button>
              <button onClick={() => { setAbaConteudo('comentarios'); carregarComentarios() }} style={abaStyleTema(abaConteudo === 'comentarios')}>Comentários</button>
            </div>
            ...
            {abaConteudo === 'informacoes' ? <div style={{ padding: '22px 2px' }}>
              ...
              {aulaAtual.descricao && <p style={{ color: cores.tx2, fontSize: '14px', lineHeight: 1.65, whiteSpace: 'pre-wrap' }}>{aulaAtual.descricao}</p>}
            ...
            </div> : <section style={{ padding: '22px 2px' }}><h3>Comentários e dúvidas</h3><p style={{ color: cores.tx3, fontSize: 13 }}>Sua mensagem é privada e a resposta aparecerá aqui.</p><form onSubmit={publicarComentario}><textarea value={novoComentario} onChange={e => setNovoComentario(e.target.value)} maxLength={1000} placeholder="Escreva sua dúvida..." style={{ width:'100%',minHeight:95,boxSizing:'border-box',background:cores.card2,color:cores.tx,border:`1px solid ${cores.borda2}`,borderRadius:10,padding:12 }} /><button disabled={enviandoComentario} style={{...botaoPrincipal,marginTop:10}}>{enviandoComentario?'Enviando...':'Enviar comentário'}</button></form><div style={{display:'grid',gap:10,marginTop:18}}>{comentarios.map(c=><article key={c.id} style={{background:cores.card,border:`1px solid ${cores.borda}`,borderRadius:10,padding:13}}><p style={{margin:0,color:cores.tx}}>{c.body}</p>{(c.respostas||[]).map(r=><div key={r.id} style={{borderLeft:`2px solid ${OURO}`,paddingLeft:10,marginTop:12}}><strong style={{color:OURO,fontSize:12}}>Resposta da Suzana</strong><p>{r.body}</p></div>)}</article>)}</div></section>}
            {materiaisAula.length > 0 && <section style={{ borderTop: `1px solid ${cores.borda}`, padding: '20px 2px' }}><h3>Materiais desta aula</h3>{materiaisAula.map(m => <button key={m.id} onClick={() => abrirMaterial(m)} style={{...botaoSecundarioTema,width:'100%',textAlign:'left',marginBottom:8}}>↗ {m.title}</button>)}</section>}

            <div style={{ display: 'flex', justifyContent: 'space-between', gap: '10px', marginTop: '18px', borderTop: '1px solid #292929', paddingTop: '20px' }}>
              <button disabled={indiceAtual <= 0} onClick={() => escolherAula(aulasPrograma[indiceAtual - 1].id)} style={{ ...botaoSecundarioTema, opacity: indiceAtual <= 0 ? .35 : 1 }}>← Aula anterior</button>
        ...
        <aside style={{ borderLeft: `1px solid ${cores.borda}`, padding: '22px 16px', background: cores.lateral }}>
          ...
          <div style={{ border: `1px solid ${cores.borda}`, borderRadius: '12px', overflow: 'hidden' }}>
            <div style={{ width: '100%', padding: '13px', background: cores.card2, color: cores.tx, fontWeight: 800 }}>▾ Mentoria {programa.toUpperCase()} <span style={{ color: cores.tx3, fontWeight: 500 }}>· {aulasPrograma.length}</span></div>
            {aulasPrograma.map((aula, i) => <button key={aula.id} onClick={() => escolherAula(aula.id)} style={{ width: '100%', padding: '12px 13px', border: 0, borderTop: `1px solid ${cores.borda}`, background: aulaAtual?.id === aula.id ? cores.ativo : cores.card, color: aulaAtual?.id === aula.id ? OURO : cores.tx2, textAlign: 'left', cursor: 'pointer', display: 'flex', gap: '9px' }}><span>{concluidas.has(String(aula.id)) ? '✓' : aula.ordem || i + 1}</span><span>{aula.titulo}</span></button>)}
          </div>
        </aside>
      </div>}
      <style jsx global>{`.mentoria-hub{max-width:1100px;margin:0 auto;padding:46px 24px}.mentoria-intro p{margin:0;color:${OURO};font-size:11px;font-weight:900;letter-spacing:.12em}.mentoria-intro h2{font-family:Georgia,serif;font-size:32px;margin:8px 0}.mentoria-intro span{color:${cores.tx2}}.mentoria-capas{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,280px));gap:20px;margin-top:28px}.mentoria-capa{padding:0;background:${cores.card};border:1px solid ${cores.borda};border-radius:15px;overflow:hidden;color:${cores.tx};text-align:left;cursor:pointer;transition:.2s}.mentoria-capa:hover{transform:translateY(-4px);border-color:${OURO}}.mentoria-capa-arte{aspect-ratio:2/3;padding:24px;position:relative;display:flex;flex-direction:column;justify-content:flex-end;background:radial-gradient(circle at 75% 20%,#735c17 0,transparent 28%),linear-gradient(145deg,#050505 38%,#2c240b 70%,#050505);overflow:hidden}.mentoria-capa-cvm .mentoria-capa-arte{background:radial-gradient(circle at 75% 20%,#6a2444 0,transparent 28%),linear-gradient(145deg,#050505 38%,#321222 70%,#050505)}.mentoria-capa-arte:before{content:'';position:absolute;inset:0;background:repeating-linear-gradient(125deg,transparent 0 26px,rgba(212,175,55,.11) 27px 28px)}.mentoria-capa-arte small,.mentoria-capa-arte strong,.mentoria-capa-arte span{position:relative;z-index:1}.mentoria-capa-arte small{color:${OURO};font-weight:900;letter-spacing:.13em}.mentoria-capa-arte strong{font-family:Georgia,serif;font-size:38px;line-height:.95;margin:13px 0}.mentoria-capa-arte span{font-size:10px;color:#bbb}.mentoria-capa-arte b{position:absolute;right:18px;top:18px;width:38px;height:38px;border-radius:50%;display:grid;place-items:center;background:rgba(0,0,0,.7);border:1px solid #777}.mentoria-capa-info{padding:15px;display:flex;flex-direction:column;gap:5px}.mentoria-capa-info span{font-size:12px;color:${cores.tx2}}@media (max-width:800px){.mentoria-layout{grid-template-columns:1fr!important}.mentoria-layout aside{border-left:0!important;border-top:1px solid ${cores.borda}!important}.mentoria-layout main{padding:18px!important}.mentoria-hub{padding:28px 18px}.mentoria-capas{grid-template-columns:repeat(2,minmax(0,1fr))}.mentoria-capa-arte{padding:15px}.mentoria-capa-arte strong{font-size:27px}}@media (max-width:520px){header strong{font-size:10px!important}.mentoria-capas{grid-template-columns:1fr}.mentoria-capa{max-width:280px}}`}</style>
    </div>
  )
}
...
function Estado({ texto, botao }) {
  ...
  return <div style={{ minHeight: '100vh', background: claro ? '#F7F6F2' : '#0A0A0A', color: claro ? '#6A6A62' : '#AAA', display: 'grid', placeItems: 'center', textAlign: 'center', padding: 20 }}><div><p>{texto}</p>{botao && <button onClick={botao} style={botaoPrincipal}>Voltar aos conteúdos</button>}</div></div>
}
